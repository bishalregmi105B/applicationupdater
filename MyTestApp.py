...
__copyright__ = 'Copyright (C) 2020,Ashlya Technology'
__version__ = '1.0'
__maintainer__ = 'Bishal Regmi'
__email__ = 'jeevanregmi48@gmail.com'
__status__ = 'Stable'
softwarename="Ashlya Updater 1.0"
_AppName_ = 'Ashlya Essays'
    ...
    def __init__(self, parent):
        def check_updates():
            try:
                ...
                response = requests.get(
                    'https://ashlyatech.lovestoblog.com/ashlyaupdater/ashlyalatestversion.txt')
                data = response.text
                ...
                if float(data) > float(__version__):
                    ...
                    if mb1 is True:
                        # -- Replace the url for your file online with the one below.
                        print("Feature Will Be come soon")
                        parent.destroy()
                    else:
                        pass
                else:
        ...
        def update_using_manager():
            try:
                ...
                # -- Replace the url for your file online with the one below.
                response = requests.get(
                    'http://ashlyatech.lovestoblog.com/ashlyaupdater/ashlyalatestversion.txt')
                data = response.text
...
class DisplayAboutMe(tk.Toplevel):
    def __init__(self, parent):
        ...
        self.geometry('{0}x{1}+{2}+{3}'.format(w, h, int(x), int(y)))
        self.resizable(width=False, height=False)
        self.title(f'About- {softwarename}')
        self.wm_iconbitmap('images/logo.ico')
        ...
        def call_link_website(*args):
            webbrowser.open_new_tab('http://brightstarpublicschool.edu.np/')

        def call_link_youtube(*args):
            webbrowser.open_new_tab('https://www.youtube.com/channel/UCcjOGhqZ14OLdSyJujo4KUg')

        tk.Label(f1, text=_AppName_ + ' ' + str(__version__)).pack()
        tk.Label(f1, text='Copyright (C) Bright Star Public School').pack()
        tk.Label(f1, text='All rights reserved').pack()

        f = font.Font(size=10, slant='italic', underline=True)
        label1 = tk.Label(f3, text='See Our Website', font=f, cursor='hand2')
        label1['foreground'] = 'blue'
        label1.pack()
        label1.bind('<Button-1>', call_link_website)
        ###############
        labe_2= tk.Label(f3, text='See Our Youtube Channel', font=f, cursor='hand2')
        labe_2['foreground'] = 'blue'
        labe_2.pack()
        labe_2.bind('<Button-1>', call_link_youtubess)
        ttk.Button(self, text='OK', command=self.destroy).pack(pady=5)


def main():
    root = tk.Tk()
    root.title(softwarename + ' ' + str(__version__))
    w = 650
    h = 400
